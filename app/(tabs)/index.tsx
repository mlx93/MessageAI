  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = getUserConversations(user.uid, async (convos) => {
        // Filter conversations based on conversation-level deletion only
        // Use conversation.deletedBy to determine if user has deleted the entire conversation
        // Use lastMessagePerUser to determine if user has any visible messages
        const filteredConversations = convos.filter(conversation => {
          // Check if user has deleted the entire conversation
          const conversationDeletedBy = conversation.deletedBy || [];
          if (conversationDeletedBy.includes(user.uid)) {
            console.log(`🗑️ Hiding conversation ${conversation.id}: User deleted conversation`);
            return false;
          }
          
          // Check if user has a valid lastMessage (either per-user or global)
          const userLastMessage = conversation.lastMessagePerUser?.[user.uid];
          const hasPerUserMessage = userLastMessage?.text && 
                                   userLastMessage.text.trim() !== '' && 
                                   userLastMessage.text !== '📷 Image' &&
                                   userLastMessage.text !== 'Photo';
          
          const hasGlobalMessage = conversation.lastMessage?.text && 
                                  conversation.lastMessage.text.trim() !== '' && 
                                  conversation.lastMessage.text !== '📷 Image' &&
                                  conversation.lastMessage.text !== 'Photo';
          
          if (!hasPerUserMessage && !hasGlobalMessage) {
            console.log(`🗑️ Hiding conversation ${conversation.id}: No valid lastMessage`);
            return false;
          }
